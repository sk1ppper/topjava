...
public class JdbcMealRepositoryImpl implements MealRepository {
    ...
    public List<Meal> getBetween(LocalDateTime startDate, LocalDateTime endDate, int userId) {
        return jdbcTemplate.query("SELECT * FROM meals WHERE userid=?", ROW_MAPPER,userId).stream().filter(meal -> meal.getDateTime().isBefore(endDate)).filter(meal -> meal.getDateTime().isAfter(startDate)).collect(Collectors.toList());
    }
}
